...
int errno;
  ...
  addr = mmap(NULL, pg_size * 4, stack_prot, flags, -1, 0);
  if (addr == MAP_FAILED) {
    printf("mmap() failed %s\n", strerror(errno));
  }

  char * end_of_stack = *((char *)(addr + (pg_size * 4)));
